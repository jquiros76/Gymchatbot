    ...
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          ...
          max_tokens: 1000,
          system: "Eres ARIA, una asistente de fitness con IA integrada en GymOS. Eres motivacional, inteligente, concisa y orientada a resultados. Hablas en español latinoamericano. Das respuestas cortas (2-3 oraciones máximo). Eres como un coach personal de alto nivel, no un bot de soporte. Incluye emojis de fitness de vez en cuando.",
          messages: [{ role: "user", content: input }]
        })
      });
      const data = await res.json();
      const text = data.content?.[0]?.text || "Vamos a por ello. ¿Qué más necesitas? 💪";
      setIsTyping(false);
      setMessages(m => [...m, {
